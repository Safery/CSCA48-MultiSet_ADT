...
class MultiSet():
    ...
    def __and__(self, multi_set):
        ...
        and the given MultiSet (New MultiSet is a 'union' of self and the 
        given MultiSet.
        '''
    ...
    def __iadd__(self, multi_set):
        '''(MultiSet, MultiSet) -> MultiSet
        Updates the original multiset so that it contains given MultiSet
        elements in self.
        '''
    ...
    def __add__(self, multi_set):
        ...
        and the given MultiSet (intersection of self and given MultiSet).
        '''
        # Initialize a new MultiSet
        self.new_set = MultiSet()
        # Select the new first Node from both of the MultiSet
    ...
    def __iand__(self, multi_set):
        '''(MultiSet, MultiSet) -> MultiSet
        Returns a modified self MultiSet that contains common element from the
        original and the given MultiSet.
        '''
        # Empty variable and a len variable to prevent self from being None
        _x = 0
        _len_self = len(self)
        # Initialize a new MultiSet
        self.new_set = MultiSet()
        # Select the first Node from both MultiSet
        current_multi_head = multi_set.skiplist.data.head
        current_head = self.skiplist.data.head
        # Loop all the way to the Tail of the self MultiSet
        while (current_head is not None):
            # If both set do not share same element
            if (str(current_head.data) not in multi_set):
                # If self is currently at second last Node
                if(_x == _len_self - 1):
                    # Return an empty self
                    self.clear()
                    return self
                else:
                    # Remove the Node from self (Since they are not same)
                    self.remove(str(current_head.data))
                    _x += 1
                    current_head = current_head.next_node
            else:
            ...
            # Returns self if both MultiSet have same elements.
            return self
